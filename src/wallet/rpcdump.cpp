...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "4. force_version        (boolean, optional, default=false) Force the version of the key to match the node\n"
    ...
    bool fRescan = true;
    {
        ...
        bool force_version = !request.params[3].isNull() && request.params[3].get_bool();

        CKey key = DecodeSecret(strSecret, force_version /* force version is equivalent to ignoring the prefix */);
        if (force_version) key.ForceValid();

        if (!key.IsValid()) throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid private key encoding; use force_version flag to import anyway");
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"          (string, required) The BitcoinSN address (or hex-encoded script)\n"
            "2. \"label\"            (string, optional, default=\"\") An optional label\n"
            "3. rescan               (boolean, optional, default=true) Rescan the wallet for transactions\n"
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid BitcoinSN address or script");
        }
    }
    if (fRescan) {
        RescanWallet(*pwallet, reserver);
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The bitcoinsn address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to bitcoinsnd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/bitcoinsn/bitcoinsn/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by BitcoinSN %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see bitcoinsnd log for details) and could "
