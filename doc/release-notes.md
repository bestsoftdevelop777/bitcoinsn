...
BitcoinSN Core version 0.17.x is now available from:

  <https://bitcoinsncore.org/bin/bitcoinsn-core-0.17.x/>
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a BSN/KB feerate (instagibbs)
...
- #9672 `46311e7` Opt-into-RBF for RPC & bitcoinsn-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) bitcoinsnd has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in bitcoinsn-cli (jnewbery)
...
- #10228 `27faa6c` build: regenerate bitcoinsn-config.h as necessary (theuni)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple bitcoinsnds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run bitcoinsn_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call BitcoinSNTestFramework start/stop node methods (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing bitcoinsnd proc (MarcoFalke)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into BitcoinSNTestFramework (jnewbery)
...
- #10090 `8e4f7e7` Update bitcoinsn.conf with example for pruning (coinables)
...
- #10447 `f259263` Make bitcoinsnd invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/bitcoinsnd.openrcconf: Don't disable wallet by default (luke-jr)
...
- #10651 `cef4b5c` Verify binaries from bitcoinsncore.org and bitcoinsn.org (TheBlueMatt)
- #10786 `ca4c545` Add PR description to merge commit in github-merge.py (sipa)
- #10812 `c5904e8` [utils] Allow bitcoinsn-cli's -rpcconnect option to be used with square brackets (jnewbery)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to bitcoinsn logs (NicolasDorier)
...
- #10958 `659c096` Update to latest BitcoinSN patches for LevelDB (sipa)
