...
static bool ProcessBlockFound(const std::shared_ptr<const CBlock> pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BitcoinSNMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(chainparams, pblock, true, NULL))
        return error("BitcoinSNMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static BitcoinSNMiner(const CChainParams& chainparams)
{
    LogPrintf("BitcoinSNMiner started\n");
    // SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinsn-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in BitcoinSNMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            std::shared_ptr<CBlock> pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);

            // LogPrintf("Running BitcoinSNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("BitcoinSNMiner terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("BitcoinSNMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateBitcoinSNs(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinSNMiner, boost::cref(chainparams)));
}
