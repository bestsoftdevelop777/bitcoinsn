 ...
 * between bitcoinsnd, and bitcoinsn-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitcoinsn-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", BITCOINSN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", BITCOINSN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
    ...
    gArgs.AddArg("-gen", strprintf(_("Generate coins (default: %u)"), DEFAULT_GENERATE), false, OptionsCategory::RPC);
    gArgs.AddArg("-genproclimit=<n>", strprintf(_("Set the number of threads for coin generation if enabled (-1 = all cores, default: %d)"), DEFAULT_GENERATE_THREADS), false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinsn/bitcoinsn>";
    const std::string URL_WEBSITE = "<https://bitcoinsncore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitcoinsn-loadblk");
...
/** Sanity checks
 *  Ensure that BitcoinSN is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-disablefork", false)) {
        printf("NOTE: disabling fork; this node will run on the bitcoinsn network\n");
        FORK_ALLOWED = false;
...
{
    // Make sure only a single BitcoinSN process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BITCOINSN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitcoinsn is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitcoinsn is started while in a temporary directory.\n",
    ...
    GenerateBitcoinSNs(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE), gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
