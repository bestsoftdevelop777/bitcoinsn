...
###   bitcoinsncore.org and bitcoinsn.org and compares them.
...
WORKINGDIR="/tmp/bitcoinsn_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinsncore.org"
HOST2="https://bitcoinsn.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinsn-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinsn-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinsn.org failed to provide signature file, but bitcoinsncore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinsn.org and bitcoinsncore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinSN Core binary release signing key installed?"
   fi
