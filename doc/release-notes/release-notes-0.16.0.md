BitcoinSN Core version 0.16.0 is now available from:

  <https://bitcoinsncore.org/bin/bitcoinsn-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/BitcoinSN-Qt` (on Mac)
or `bitcoinsnd`/`bitcoinsn-qt` (on Linux).
...
BitcoinSN Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

BitcoinSN Core should also work on most other Unix-like systems but is not
...
BitcoinSN Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
bitcoinsn data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile BitcoinSN Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µBSN" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- bitcoinsn-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- bitcoinsn-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest bitcoinsn.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.bitcoinsn.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for bitcoinsn-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to bitcoinsn-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µBSN to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_bitcoinsn_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` bitcoinsn_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.bitcoinsn doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for bitcoinsn-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in bitcoinsn-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop bitcoinsnd (OmeGak)
...
- #10529 `331352f` Improve bitcoinsnd systemd service file (Flowdalic)
