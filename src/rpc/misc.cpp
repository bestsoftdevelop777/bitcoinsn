...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bitcoinsn balance of the wallet\n"
    ...
    obj.push_back(Pair("deprecation-warning", "WARNING: getinfo is deprecated and will be fully removed SOON."
                                              " Projects should transition to using getblockchaininfo, getnetworkinfo, and getwalletinfo ASAP"));
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The bitcoinsn address to use for the signature.\n"
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "bitcoinsn-cli and the GUI. There is no server-side difference.");
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\nbitcoinsn-cli has the option -getinfo to collect and format these in the old format.");
}
...
static const CRPCCommand commands[] =
    {
        ...
        {"forkgen", "getinfo", &getinfo, {}}, /* uses wallet if enabled */
