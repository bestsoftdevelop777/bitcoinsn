...
const char* const BITCOINSN_CONF_FILENAME = "bitcoinsn.conf";
const char* const BITCOINSN_PID_FILENAME = "bitcoinsnd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinsnd -foo=bar
...
#else
    const char* pszModule = "bitcoinsn";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinSN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinSN
    // Mac: ~/Library/Application Support/BitcoinSN
    // Unix: ~/.bitcoinsn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinSN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinSN";
#else
    // Unix
    return pathRet / ".bitcoinsn";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINSN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINSN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure BitcoinSN Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BitcoinSN Core") == std::string::npos) {
