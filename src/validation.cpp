...
#if defined(NDEBUG)
#error "BitcoinSN cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "BitcoinSN Signed Message:\n";
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool, unsigned int flags, bool cacheSigStore, PrecomputedTransactionData& txdata)
{
    AssertLockHeld(cs_main);
...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting) {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BSN additional fees, %d delta bytes\n",
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount nAbsurdFee, bool test_accept)
{
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (pindex->nHeight == FORK_BLOCK + !FORK_BLOCK) {
        // undo premine tx
        CAmount premine_amount = (CAmount)10500000000000;
        CTxDestination txd(CPubKey(ParseHex("04D9B17A452995CF44C2B0D12D42DD745DAA58ABB76D073A0B8F7DFBF9CA85ECBF8663F275F8BF6769490014E8")).GetID());
        CScript dest = GetScriptForDestination(txd);

        // add premine tx, which is also a coinbase tx (in other words, the fork block has TWO coinbase transactions)
        ...
        if (!is_spent || premine_tx.vout[0] != coin.out || pindex->nHeight != coin.nHeight || !coin.fCoinBase) {
            // fClean = false; // transaction output mismatch
            fprintf(stderr, "* premine undo failure! UTXO set is tainted and may need resync\n");
        }
    }
...
{
    RenameThread("bitcoinsn-scriptch");
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    bool premine_block = pindex->nHeight == (FORK_BLOCK + !FORK_BLOCK);
    if (premine_block) printf("** premine block; will add to UTXO set **\n");
