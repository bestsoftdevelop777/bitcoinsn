...
inline std::vector<OutputGroup>& GroupCoins(const std::vector<COutput>& coins)
{
    ...
    static_groups.clear();
    for (auto& coin : coins)
        static_groups.emplace_back(coin.GetInputCoin(), coin.nDepth, coin.tx->fDebitCached && coin.tx->nDebitCached == 1 /* HACK: we can't figure out the is_me flag so we use the conditions defined above; perhaps set safe to false for !fIsFromMe in add_coin() */, 0, 0);
    return static_groups;
}
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++) {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN); // we should get 1 BSN in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN); // we should get 2 BSN in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
